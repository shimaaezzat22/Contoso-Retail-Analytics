...
SELECT
    ...
    CAST(CASE
        ...
        ELSE 0
    END AS BIT) AS IsDigital,            -- BIT: 1 = real-time gateway, 0 = offline/manual
    ac.EstimatedCACLow,         -- MONEY: lower bound of CAC range
    ac.EstimatedCACHigh,        -- MONEY: upper bound of CAC range
    -- Midpoint for single-value CAC calculations in DAX
    -- Used MONEY to match the precision of the source columns and ensure consistency in DAX measures.
    CAST((ac.EstimatedCACLow + ac.EstimatedCACHigh) / 2.0 AS MONEY)
                                                            AS CACMidpoint
