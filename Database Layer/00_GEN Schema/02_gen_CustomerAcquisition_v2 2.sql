...
--------------------------------------------------------------------------------
  SCRIPT OBJECTIVE
--------------------------------------------------------------------------------
  1. Generate gen.CustomerAcquisition — a one-row-per-Person-customer table
     that assigns each qualifying customer in dbo.DimCustomer to exactly one
     acquisition channel from gen.DimAcquisitionChannel.

  2. Embed discoverable, realistic acquisition patterns into the data using a
     5-factor competitive scoring model (BaseWeight × YearFactor × AgeFactor ×
     IncomeFactor × GeoFactor × RandomNoise), so students can surface
     meaningful channel insights in Power BI rather than uniform noise.

  3. Store AcquisitionDate as the raw source DateFirstPurchase (no +16 year
     shift applied at the [gen] layer), preserving the architectural principle
     that ALL temporal transformations happen exclusively at the [fact] view
     layer. This eliminates any double-shifting risk.
  ...
  no channel attribution, no CAC benchmarking, no paid/organic mix, and no
  channel-to-CLV analysis.
...
-- ============================================================================
DECLARE @MinYear INT;    -- Earliest DateFirstPurchase year across all qualifying customers.
DECLARE @MaxYear INT;    -- Latest DateFirstPurchase year across all qualifying customers.
DECLARE @YearRange FLOAT; -- Computed denominator for YearProgress normalisation (0.0 → 1.0 range).
                           -- FLOAT (not INT) because it is used as a division denominator below.

-- Pre-calculation pattern: MIN/MAX are computed BEFORE the CTE chain begins.
-- This avoids a Table Spool on dbo.DimCustomer if these aggregates were computed inside
-- the CTE body. Pre-declaring scalars forces a single scan here; the CTE then reads
-- the scalar directly without re-scanning the table. Critical for large customer datasets.
SELECT 
    @MinYear = MIN(YEAR(DateFirstPurchase)),   -- YEAR() extracts the 4-digit year from the DATE column.
    @MaxYear = MAX(YEAR(DateFirstPurchase))    -- Both aggregated in a single scan for efficiency.
FROM [dbo].[DimCustomer]
WHERE CustomerType = 'Person'         -- Exclude B2B company accounts — no acquisition channel applies.
  AND DateFirstPurchase IS NOT NULL;  -- NULL DateFirstPurchase would produce NULL AcquisitionYear/YearProgress.

-- NULLIF prevents division by zero when the entire dataset falls within a single calendar year.
-- If @MaxYear = @MinYear, the subtraction produces 0. NULLIF converts that 0 to NULL,
-- which propagates through the YearProgress division as NULL rather than a runtime error.
-- The YearProgress expression in the CTE uses NULL-safe arithmetic (NULL → 0.5 default in scoring).
SET @YearRange = NULLIF(@MaxYear - @MinYear, 0);

;WITH CustomerProfile AS (

    SELECT
        c.CustomerKey,                                      -- PK from dbo.DimCustomer. Will become the PK of gen.CustomerAcquisition.
        c.DateFirstPurchase                         AS AcquisitionDate,  -- Raw source date. No +16 shift applied here — shift is at fact.vCustomerAcquisition.
        YEAR(c.DateFirstPurchase)                   AS AcquisitionYear,  -- Calendar year used for YearProgress normalisation below.

        -- ── Age at time of first purchase (birthday-corrected) ───────────────
        -- Best practice: DATEDIFF(YEAR,...) alone over-counts age by 1 for people
        -- whose birthday falls AFTER the reference date in that year. The CASE
        -- block corrects this: DATEADD(YEAR, fullYears, BirthDate) reconstructs
        -- the most recent birthday. If it falls after AcquisitionDate, the person
        -- hasn't had their birthday yet this year — subtract 1 from the year diff.
        DATEDIFF(YEAR, c.BirthDate, c.DateFirstPurchase)
            - CASE
                ...
                THEN 1   -- Birthday hasn't occurred yet this year → subtract 1.
                ELSE 0   -- Birthday has already occurred → no adjustment needed.
              END                                   AS AgeAtAcquisition,

        c.YearlyIncome,                                     -- Raw income value from source. Used for IncomeFactor band mapping in ChannelScores.
        ISNULL(g.ContinentName, 'Unknown')          AS Continent,
        -- ISNULL guard: LEFT JOIN to DimGeography may return NULL if a customer
        -- has no GeographyKey match. 'Unknown' maps to the ELSE branch in GeoFactor.

        -- ── YearProgress: normalised temporal position (0.0 → 1.0) ─────────
        -- 0.0 = earliest first-purchase year in the dataset (e.g. 2007)
        -- 1.0 = latest  first-purchase year in the dataset (e.g. 2009)
        -- CAST to FLOAT ensures true decimal division (INT/INT would truncate to 0 or 1).
        -- Feeds the YearFactor in ChannelScores to encode how channel mix evolved over time.
        -- @YearRange is pre-computed as NULLIF(...,0) — NULL propagates safely if range = 0.
       CAST(YEAR(c.DateFirstPurchase) - @MinYear AS FLOAT) 
        / @YearRange AS YearProgress

    FROM [dbo].[DimCustomer]   AS c
    LEFT JOIN [dbo].[DimGeography] AS g    -- LEFT JOIN: preserves all customers even if GeographyKey is unmatched.
        ON c.GeographyKey = g.GeographyKey -- Standard geography FK join. ISNULL on ContinentName handles NULL result.

    WHERE c.CustomerType        = 'Person'      -- Exclude B2B / company accounts (no acquisition channel applies).
      AND c.DateFirstPurchase   IS NOT NULL      -- AcquisitionDate is required — NULL would produce NULL YearProgress.
      AND c.BirthDate           IS NOT NULL      -- AgeFactor requires DOB — NULL BirthDate would break birthday correction.
    ...
    SELECT
        ...
        CAST(
            ...
            * CASE ch.AcquisitionChannelKey
            ...
            -- CHECKSUM(NEWID()) produces a unique non-deterministic integer per row.
            -- ABS() removes negative values. Modulo 1000 bounds to 0–999.
            -- Dividing by 1000.0 produces a 0.000–0.999 decimal.
            -- Adding 0.5 shifts the range to 0.500–1.499. This multiplier can tip
            -- borderline cases between channels — which is exactly what we want.
            -- Each execution produces different individual assignments (by design)
            -- while population-level patterns (age/income/geo signals) remain stable.
            * (0.5 + (ABS(CHECKSUM(NEWID())) % 1000) / 1000.0)

        AS FLOAT) AS FinalScore   -- CAST to FLOAT: all factors are FLOAT/DECIMAL — consistent precision throughout.

    FROM CustomerProfile          AS cp
    CROSS JOIN [gen].[DimAcquisitionChannel] AS ch
    -- Best practice: CROSS JOIN is intentional and correct here. We need every customer
    -- scored against EVERY channel so the competition is genuine. This produces
    -- N_customers × 7 rows. Only the top-scored row per customer is inserted.
    -- A developer seeing CROSS JOIN should understand it is a deliberate design choice,
    -- not a missing JOIN condition. The volume is controlled: 7 channels × ~18K customers
    -- = ~126K rows — fully in-memory, no spill to tempdb.
...
RankedChannels AS (
    ...
    SELECT
        ...
        FinalScore,
        ROW_NUMBER() OVER (
            PARTITION BY CustomerKey   -- Rank channels independently per customer. Each customer gets their own rank sequence.
            ORDER BY FinalScore DESC   -- Highest score = Rank 1 = winning channel.
        ) AS Rank
        -- Best practice: ROW_NUMBER() (not RANK() or DENSE_RANK()) is used deliberately.
        -- ROW_NUMBER() guarantees unique integers with no ties — exactly one row per
        -- CustomerKey gets Rank=1. RANK() would assign the same rank to tied rows,
        -- potentially inserting two rows for the same customer (violating the PK).
        -- In practice, ties are extremely rare given the noise factor, but using
        -- ROW_NUMBER() makes the uniqueness guarantee structural, not probabilistic.

    FROM ChannelScores

)

-- Final insert: only the winning channel (Rank = 1) per customer is written to the table.
-- WHERE Rank = 1 filters N_customers × 7 scored rows down to exactly N_customers rows.
-- The PK constraint on CustomerKey in gen.CustomerAcquisition enforces the one-row-per-customer
-- guarantee at the database level as a second safety net.
...
SELECT
    ...
    AcquisitionDate
FROM RankedChannels
WHERE Rank = 1;  -- Winning channel only. Exactly one row per customer.
...
-- ║  ─────────────────────────────────────────────────────────────────────     ║
-- ║  ① Non-Clustered Index on the FK column (AcquisitionChannelKey)            ║
-- ║    The table's clustered index is on CustomerKey (the PK). Joining to      ║
-- ║    dim.vAcquisitionChannel on AcquisitionChannelKey requires a full scan   ║
-- ║    of the clustered index without this NCI. The NCI provides a direct      ║
-- ║    seek to all rows with a given ChannelKey — reducing lookup cost from    ║
-- ║    O(n) scan to O(log n) seek.                                             ║
-- ║                                                                             ║
-- ║  ② INCLUDE (AcquisitionDate) — covering index pattern                      ║
-- ║    fact.vCustomerAcquisition always SELECTs AcquisitionDate alongside the  ║
-- ║    key. Without the INCLUDE clause SQL Server performs a Key Lookup back   ║
-- ║    to the clustered index (CustomerKey PK) to retrieve AcquisitionDate —  ║
-- ║    a separate random IO per matched row. The INCLUDE column makes the NCI  ║
-- ║    a COVERING index: all columns needed by the query are in the index      ║
-- ║    leaf pages, eliminating the Key Lookup entirely.                        ║
-- ║    Rule: Include any non-key column that is always SELECTed alongside      ║
-- ║    the index key in the primary access pattern for that table.             ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
-- STEP 4: Performance index on AcquisitionChannelKey (FK join column)
-- ============================================================================

CREATE NONCLUSTERED INDEX [IX_CustomerAcquisition_ChannelKey]
    ON [gen].[CustomerAcquisition] (AcquisitionChannelKey)   -- Index key: the FK join column used by fact.vCustomerAcquisition → dim.vAcquisitionChannel.
    INCLUDE (AcquisitionDate);                                -- Covering: eliminates Key Lookup by including the always-selected date column in index leaf pages.
...
-- ║  BEST PRACTICES APPLIED                                                     ║
-- ║  ─────────────────────────────────────────────────────────────────────     ║
-- ║  ① CASE WHEN...THEN 1 ELSE 0 END + SUM for multi-metric single-scan        ║
-- ║    V1 and V2 compute TotalRows, NullCount, and MinMax in a single SELECT   ║
-- ║    pass using conditional SUM (not multiple COUNT queries). This halves    ║
-- ║    the table scans compared to separate COUNT(NULLIF(...)) calls.          ║
-- ║                                                                             ║
-- ║  ② CAST percentage with denominator-safe NULLIF                            ║
-- ║    V3 computes ChannelPct% using CAST(100.0 * COUNT / NULLIF(total, 0)).   ║
-- ║    NULLIF ensures no division-by-zero if the table is empty.               ║
-- ║    100.0 forces floating-point division; integer/integer would truncate.   ║
-- ║                                                                             ║
-- ║  ③ NOT EXISTS for referential integrity check (V7)                          ║
-- ║    V7 uses NOT EXISTS to find orphaned rows — CustomerAcquisition rows      ║
-- ║    whose AcquisitionChannelKey has no matching row in DimAcquisitionChannel.║
-- ║    NOT EXISTS is more efficient than an outer join + IS NULL pattern for   ║
-- ║    existence checks.                                                        ║
-- ║                                                                             ║
