...
-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║  CODE BLOCK 4 — STEP 3: 5-STAGE CTE PIPELINE + INSERT                   ║
-- ╠═══════════════════════════════════════════════════════════════════════════╣
-- ║                                                                           ║
-- ║  WHAT THIS DOES                                                           ║
-- ║  Identical pipeline structure to Script 07, adapted for FactSales:      ║
-- ║                                                                           ║
-- ║  CTE 0: EligibleReasons — pre-calculate modulo index for 'Both'-only    ║
-- ║    reasons ONCE. Prevents a correlated N+1 Table Spool in FinalRows     ║
-- ║    by materialising gen.DimReturnReason (AppliesTo = 'Both' filter)     ║
-- ║    into a pre-ranked, pre-counted set before the main pipeline runs.    ║
-- ║    KEY DIFFERENCE FROM SCRIPT 07: AppliesTo = 'Both' ONLY here.         ║
-- ║    'Online Only' reasons (e.g., Late Delivery) are meaningless for       ║
-- ║    in-store transactions — the customer physically brought the item back.║
-- ║                                                                           ║
...
-- ║    FinalRows joins CTE 0 ([EligibleReasons]) to resolve ReturnReasonKey  ║
-- ║    via: [nl].[NoiseB] % [er].[TotalEligible] = [er].[RN]                ║
-- ║                                                                           ║
...
[NoiseLocked] AS (
    SELECT
        [vt].*,
        -- We replace NEWID() with a deterministic hash of the PK + a salt.
        -- This absolutely prevents the optimizer from generating multiple values
        -- for the same row during downstream joins.
        ABS(CHECKSUM([vt].[SourceSalesKey], 'A')) AS [NoiseA],
        ABS(CHECKSUM([vt].[SourceSalesKey], 'B')) AS [NoiseB],
        ABS(CHECKSUM([vt].[SourceSalesKey], 'C')) AS [NoiseC]
    FROM [ValueTiered] AS [vt]
    -- Note: We completely removed the CROSS APPLY for the noise generation.
),
