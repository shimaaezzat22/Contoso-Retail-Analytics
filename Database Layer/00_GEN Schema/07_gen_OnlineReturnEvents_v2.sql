...
-- ║                                                                           ║
-- ║  WHAT THIS DOES                                                           ║
-- ║  Generates all return event rows via a 5-CTE pipeline that enriches      ║
...
-- ║  │    Pre-calculates the full modulo index for return reason selection  │ ║
-- ║  │    ONCE at the start of the pipeline. Filters gen.DimReturnReason   │ ║
-- ║  │    to AppliesTo IN ('Both', 'Online Only'). Assigns a 0-based RN    │ ║
-- ║  │    and a TotalEligible count via window functions.                   │ ║
-- ║  │    Without this CTE, the reason-selection subquery in FinalRows     │ ║
-- ║  │    would execute as a correlated N+1 subquery — one full scan of    │ ║
-- ║  │    DimReturnReason per return event row, causing a Table Spool.     │ ║
-- ║  │                                                                     │ ║
-- ║  │  CTE 1: SourceLines                                                 │ ║
...
-- ║  │    Joins CTE 0 ([EligibleReasons]) to resolve ReturnReasonKey       │ ║
...
-- ║  ReturnReasonKey is selected via a modulo index into the pre-built      ║
-- ║  CTE 0 ranked subset of gen.DimReturnReason filtered by AppliesTo      ║
-- ║  IN ('Both', 'Online Only'). This design is resilient to future        ║
-- ║  additions to DimReturnReason — no code change is needed if new        ║
-- ║  reasons are added to the dimension table.                              ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
